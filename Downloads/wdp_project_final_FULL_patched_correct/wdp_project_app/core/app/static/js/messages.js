...
function showProfanityBlock(text) {
  ...
  async function apiListMessages(chatId) {
    const res = await fetch(`/api/messages/${encodeURIComponent(chatId)}`, { headers: demoHeaders() });
  ...
  async function apiGetSession() {
    const res = await fetch("/api/session", { headers: demoHeaders() });
  ...
  async function apiDeleteMessage(messageId) {
    const res = await fetch(`/api/messages/${messageId}`, { method: "DELETE", headers: demoHeaders() });
  ...
  async function apiRestoreMessage(messageId) {
    const res = await fetch(`/api/messages/${messageId}/restore`, { method: "POST", headers: demoHeaders() });
